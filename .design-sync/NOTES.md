...
- **Brand assets are also off-script**: `site/assets/room-and-board-{wordmark-dark.svg,lockup-dark.svg,icon-180.png,favicon-32.png}` are copied into `ds-bundle/brand/` after every build (same reason as the cards). `BrandAssets.html` references them via `../brand/`, and conventions.md documents them for the design agent. Keep the img width/height at the true aspect (wordmark 216x50 from 268x62; lockup 216x56 from 373x96).
