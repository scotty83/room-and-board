...
const MODELS = {
  ...
  // Markets rows are width-sensitive: the 3-wide stacked variant compacts to
  // ~50px rows + 8px gaps (data-w=3 CSS), the 4-wide classic runs the 69px
  // pitch, and shallow spark-less rows 36 (fits all 3 tickers at 3x2).
  // Browser-calibrated (6-ticker fixture, w3/w4 x h3-6): a 3x4 now shows 5.
  markets: (w, h) =>
    Math.max(1, sizeTier(h) === 's'
      ? Math.floor(bodyPx(h) / 36)
      : w <= 3
        ? Math.floor((bodyPx(h) + 8) / 58)
